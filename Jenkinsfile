...
pipeline {
  ...
  stages {
    stage('Build and push image') {
      ...
      steps {
        sh '''
          docker build --no-cache --pull -t ${IMAGE_NAME}:${GIT_COMMIT_SHORT}-${BUILD_NUMBER} -t ${IMAGE_NAME}:latest .
        '''
        ...
        script {
          openshift.withCluster('c1-ci.eclipse.org') {
            openshift.withProject("${NAMESPACE}") {
              ...
              app.spec.template.spec.containers[1].image = "${IMAGE_NAME}"+':'+"${GIT_COMMIT_SHORT}-${BUILD_NUMBER}"
              openshift.apply(app)
              timeout(5) {
